...
export const api: TmsApi = {
  customers: token => request<Customer[]>('/api/v2/customers', token),
  customerContacts: token => request<CustomerContact[]>('/api/v2/customer-contacts', token),
  vehicles: token => request<Vehicle[]>('/api/v2/vehicles', token),
  updateVehicle: (id, payload, token) => request<Vehicle>(`/api/v2/vehicles/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  drivers: token => request<Driver[]>('/api/v2/drivers', token),
  updateDriver: (id, payload, token) => request<Driver>(`/api/v2/drivers/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  trailers: token => request<Trailer[]>('/api/v2/trailers', token),
  sites: token => request<Site[]>('/api/v2/sites', token),
  updateSite: (id, payload, token) => request<Site>(`/api/v2/sites/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  marketContacts: async token => normaliseMarketContacts(await request<MarketContact[]>('/api/v2/market-contacts', token)),
  updateMarketContact: (id, payload, token) => request<MarketContact>(`/api/v2/market-contacts/${id}`, token, { method: 'PUT', body: JSON.stringify(payload) }),
  fuelPrices: token => request<FuelPrice[]>('/api/v2/fuel-prices', token),
  saveFuelPrice: (payload, token) => request<FuelPrice>('/api/v2/fuel-prices', token, { method: 'POST', body: JSON.stringify(payload) }),
  staging: (token, status = 'PendingReview', entityType = '', take = 100) => request<StagedImport[]>(`/api/v2/staging?take=${Math.min(Math.max(take, 1), 200)}${status ? `&status=${encodeURIComponent(status)}` : ''}${entityType ? `&entityType=${encodeURIComponent(entityType)}` : ''}`, token),
  orders: (from, to, token) => request<TransportOrder[]>(`/api/v2/orders?${new URLSearchParams({ ...(from ? { from } : {}), ...(to ? { to } : {}) })}`, token),
  stageOrder: (payload, idempotencyKey, token) => request<StageImportResponse>('/api/v2/staging', token, { method: 'POST', body: JSON.stringify({ entityType: 'order', idempotencyKey, source: 'SLH TMS Web/CSV', payload }) }),
  stageRecord: (entityType, payload, idempotencyKey, token) => request<StageImportResponse>('/api/v2/staging', token, { method: 'POST', body: JSON.stringify({ entityType, idempotencyKey, source: 'SLH TMS Web', payload }) }),
  stageBatch: (records, token) => request<StageBatchResponse>('/api/v2/staging/batch', token, { method: 'POST', body: JSON.stringify(normaliseMarketMasterRecords(records)) }),
  applyMasterData: (records, token) => request<MasterApplyResponse>('/api/v2/master-data/apply', token, { method: 'POST', body: JSON.stringify(normaliseMarketMasterRecords(records)) }),
  linkMasterRegister: token => request<{ linked: number; message: string }>('/api/v2/master-data/register/link', token, { method: 'POST' }),
  telemetry: token => request<Telemetry>('/api/v2/tracking/dot/telemetry', token),
  fleetStatus: token => request<FleetStatus>('/api/v2/tracking/dot/fleet-status', token),
  trackingHistory: (date, token) => request<Telemetry>(`/api/v2/tracking/dot/history?date=${encodeURIComponent(date)}`, token),
  operationsReconciliation: (date, token) => request<OperationsReconciliation>(`/api/v2/operations/reconciliation?date=${encodeURIComponent(date)}`, token),
  operationsExceptions: (date, token) => request<OperationsExceptions>(`/api/v2/operations/exceptions?date=${encodeURIComponent(date)}`, token),
  driverAssignments: (from, to, token) => request<DriverAssignment[]>(`/api/v2/driver-assignments?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`, token),
  returnLoadSuggestions: (date, token) => request<ReturnLoadSuggestions>(`/api/v2/planning/return-load-suggestions?date=${encodeURIComponent(date)}`, token),
  deliveryEtas: (date, token) => request<DeliveryEtas>(`/api/v2/operations/delivery-etas?date=${encodeURIComponent(date)}`, token),
  sendDispatchSms: (loadId, token) => request<DispatchSmsResponseDto>(`/api/v2/loads/${loadId}/dispatch/sms`, token, { method: 'POST' }),
  geocode: (address, token) => geocodeAddress(address, token),
  sageHrStatus: token => request<SageHrStatus>('/api/v2/integrations/sage-hr/status', token),
  tachoMasterStatus: token => request<TachoMasterStatus>('/api/v2/integrations/tachomaster/status', token),
  syncTachoMasterDrivers: token => request<TachoMasterSync>('/api/v2/integrations/tachomaster/sync-drivers', token, { method: 'POST' }),
  roadTechStatus: token => request<RoadTechStatus>('/api/v2/integrations/roadtech/status', token),
  fleetioStatus: token => request<FleetioStatus>('/api/v2/integrations/fleetio/status', token),
  fleetioVehicleAlignment: token => request<FleetioVehicleAlignment>('/api/v2/integrations/fleetio/vehicle-alignment', token),
  syncFleetioVehicles: token => request<FleetioSync>('/api/v2/integrations/fleetio/sync-assets-resilient', token, { method: 'POST' }),
  integrationStatus: token => request<IntegrationStatus>('/api/v2/integrations/status', token),
  intakeHealth: token => request<IntakePipelineHealth>('/api/v2/health/intake', token),
  pollMailboxNow: token => request('/api/v2/health/intake/poll', token, { method: 'POST' }),
  diagnosticsTables: token => request<DiagnosticsTables>('/api/v2/diagnostics/tables', token),
  syncSageHrDrivers: token => request<SageHrSync>('/api/v2/integrations/sage-hr/sync-drivers', token, { method: 'POST' }),
  customerCommunications: (token, status = 'PendingReview', purpose, take = 100) => request<CustomerCommunication[]>(`/api/v2/customer-communications?${new URLSearchParams({ ...(status ? { status } : {}), ...(purpose ? { purpose } : {}), take: String(take) })}`, token),
  approveCustomerCommunication: (id, note, token) => request(`/api/v2/customer-communications/${id}/approve`, token, { method: 'POST', body: JSON.stringify({ note }) }),
  rejectCustomerCommunication: (id, note, token) => request(`/api/v2/customer-communications/${id}/reject`, token, { method: 'POST', body: JSON.stringify({ note }) }),
  duplicateCheck: (payload, token) => request<DuplicateCheckResponse>('/api/v2/order-intake/duplicate-check', token, { method: 'POST', body: JSON.stringify(payload) }),
  review: (id, approved, note, token) => request(`/api/v2/staging/${id}/${approved ? 'approve' : 'reject'}`, token, { method: 'POST', body: JSON.stringify({ note }) }),
  clearPendingStaging: token => request<{ deleted: number }>('/api/v2/staging/pending?confirm=CLEAR-PENDING', token, { method: 'DELETE' }),
  assistantSnapshot: (date, token) => request<AssistantSnapshot>(`/api/v2/assistant/snapshot?date=${encodeURIComponent(date)}`, token),
  assistantAdvice: (message, date, token) => request<AssistantAdvice>('/api/v2/assistant/advice', token, { method: 'POST', body: JSON.stringify({ message, date }) }),
  fixSafeValidations: token => request<SafeFixResult>('/api/v2/assistant/fix-safe-validations', token, { method: 'POST' }),
};
