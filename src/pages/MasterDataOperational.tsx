...
const config: Record<MasterDataTab, { title: string; search: string; entityType: string; columns: Array<[string, string]>; editable: Array<[string, string, EditType]> }> = {
  ...
  sites: { title: 'Sites', search: 'Site name, alias, SITE code, geofence, postcode, region or driver text name', entityType: 'Site', columns: [['name','Site'],['externalCode','Code'],['linkedGeofence','Linked geofence'],['driverTextName','Driver text'],['collectionAddress','Address / postcode'],['defaultTemperatureC','Default temp'],['region','Planning region']], editable: [['name','Site name','text'],['driverTextName','Driver text name','text'],['aliases','Planner / customer aliases (comma or semicolon separated)','textarea'],['collectionAddress','Address / postcode','textarea'],['collectionInstructions','Collection instructions / notes','textarea'],['mapLink','Map link','text'],['defaultTemperatureC','Default temperature °C','number'],['region','Planning region','region']] },
    ...
    {selected && (crmMode ? <div className="crm-modal-backdrop" role="dialog" aria-modal="true" aria-label={`Edit ${current.entityType}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(undefined); }}>
      <div className="crm-modal">
        <div className="crm-modal-header">
          <div><p className="eyebrow">{current.entityType} CRM record</p><h2>{fmt(selected[current.columns[0][0]])}</h2><p className="hint">{tab === 'sites' ? 'Site is the master physical location. Add every planner/customer wording as an alias; all aliases then resolve to this Site and its confirmed geofence.' : 'Edit the customer master record and supporting documents from one place.'}</p></div>
