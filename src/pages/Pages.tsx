...
export function StagingQueue() {
  const token = useAccessToken(); const [entityFilter, setEntityFilter] = useState(''); const load = useCallback(async () => api.staging(await token(), 'PendingReview', entityFilter, 2000), [token, entityFilter]); const { data, loading, error, refresh } = useApi(load); const [reviewing, setReviewing] = useState<string>(); const [selected, setSelected] = useState<StagedImport>(); const [bulkEntity, setBulkEntity] = useState('vehicle'); const [bulkMessage, setBulkMessage] = useState<string>(); const [requestingOrders, setRequestingOrders] = useState(false);
  ...
  async function requestOrders() { setRequestingOrders(true); setBulkMessage(undefined); try { const result = await api.pollMailboxNow(await token()); setBulkMessage(`${result.message} ${result.lastMessagesIngested} new record${result.lastMessagesIngested === 1 ? '' : 's'} staged.`); await refresh(); } catch (exception) { setBulkMessage(exception instanceof Error ? exception.message : 'Orders could not be requested from Microsoft Graph.'); } finally { setRequestingOrders(false); } }
  ...
  const orderReviewCount = (data || []).filter(item => item.entityType === 'order').length;
