...
type StagedOrderReadiness = { collection: string; delivery: string; pallets: string; ready: boolean; reason: string };
function stagedPayload(item: StagedImport): Record<string, unknown> {
...
function payloadText(payload: Record<string, unknown>, ...keys: string[]) {
  for (const key of keys) { const value = payload[key]; if (value !== undefined && value !== null && String(value).trim()) return String(value).trim(); }
  return '';
}
function stagedOrderReadiness(item: StagedImport): StagedOrderReadiness | undefined {
  if (item.entityType !== 'order') return undefined;
  const payload = stagedPayload(item);
  const collection = payloadText(payload, 'collectionSite', 'collectionLocation', 'sellerName', 'collectionAddress');
  const delivery = payloadText(payload, 'deliverySite', 'deliveryLocation', 'destination', 'stallNumber', 'deliveryAddress');
  const pallets = payloadText(payload, 'pallets', 'palletQty', 'palletQuantity', 'quantity');
  const warnings = Array.isArray(payload.intakeWarnings) ? payload.intakeWarnings.filter(Boolean) : [];
  const hasPallets = Number(pallets) > 0 || /backhaul|backload/i.test(payloadText(payload, 'jobType'));
  const plannerReady = payload.plannerReady === true || payload.plannerReady === 'true';
  const highConfidence = payloadText(payload, 'intakeConfidence').toLowerCase() === 'high';
  const ready = Boolean(collection && delivery && hasPallets && plannerReady && highConfidence && warnings.length === 0);
  const reason = ready ? 'Route, quantity and master-data checks passed.' : warnings[0] ? String(warnings[0]) : !collection ? 'Collection point is missing or not matched.' : !delivery ? 'Delivery point is missing or not matched.' : !hasPallets ? 'Pallet quantity is missing.' : !highConfidence ? 'Intake confidence requires planner review.' : 'Master-data or planner-readiness checks are incomplete.';
  return { collection: collection || 'Not supplied', delivery: delivery || 'Not supplied', pallets: pallets || '—', ready, reason };
}
...
export function StagingQueue() {
  const token = useAccessToken(); const load = useCallback(async () => api.staging(await token(), 'PendingReview', 'order', 2000), [token]); const { data, loading, error, refresh } = useApi(load); const [reviewing, setReviewing] = useState<string>(); const [selected, setSelected] = useState<StagedImport>(); const [message, setMessage] = useState<string>(); const [requestingOrders, setRequestingOrders] = useState(false); const [sourceEvidenceId, setSourceEvidenceId] = useState<string>();
  async function review(item: StagedImport, approved: boolean) { setReviewing(item.id); setMessage(undefined); try { await api.review(item.id, approved, '', await token()); setMessage(approved ? 'Order approved and promoted to Planner Builder.' : 'Order rejected and removed from the review queue.'); setSelected(undefined); await refresh(); } catch (exception) { setMessage(exception instanceof Error ? exception.message : 'The order could not be approved. Check the readiness reason and source evidence.'); } finally { setReviewing(undefined); } }
  async function requestOrders() { setRequestingOrders(true); setMessage(undefined); try { const result = await api.pollMailboxNow(await token()); setMessage(`${result.message} ${result.lastMessagesIngested} new order${result.lastMessagesIngested === 1 ? '' : 's'} staged.`); await refresh(); } catch (exception) { setMessage(exception instanceof Error ? exception.message : 'Orders could not be requested from Microsoft Graph.'); } finally { setRequestingOrders(false); } }
  const payload = selected ? (() => { try { return JSON.parse(selected.payloadJson) as Record<string, unknown>; } catch { return {}; } })() : undefined;
  const orderReadiness = useMemo(() => new Map((data || []).map(item => [item.id, stagedOrderReadiness(item)])), [data]);
  const clearForApproval = (data || []).filter(item => orderReadiness.get(item.id)?.ready).length;
  const orderReviewCount = data?.length || 0;
  return <section><div className="title-row"><div><p className="eyebrow">Order intake</p><h1>Order Review</h1><p className="hint">Orders are pulled from the Microsoft Graph mailbox, checked against the order and site master data, then held here for planner approval.</p></div><div className="actions"><button onClick={() => void refresh()}>Refresh</button><button className="primary" disabled={requestingOrders} onClick={() => void requestOrders()}>{requestingOrders ? 'Requesting...' : 'Request orders now'}</button></div></div><div className="planner-toolbar staging-filter"><span>{orderReviewCount} order{orderReviewCount === 1 ? '' : 's'} awaiting review</span>{message && <span className="notice inline-notice">{message}</span>}</div><State loading={loading} error={error} empty={!data?.length}><div className="review-readiness-summary"><strong>{orderReviewCount} order{orderReviewCount === 1 ? '' : 's'} in review</strong><span>{clearForApproval} clear for approval</span><span>{Math.max(orderReviewCount - clearForApproval, 0)} need route, quantity or master-data review</span></div>{selected && <div className="review-modal-scrim" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setSelected(undefined); }}><div className="panel review-panel review-modal" role="dialog" aria-modal="true" aria-label="Order review details"><div><p className="eyebrow">Reviewing order</p><h2>{String(payload?.poNumber || payload?.name || selected.id)}</h2></div><div className="actions"><button onClick={() => setSourceEvidenceId(selected.id)}>View email & attachments</button><button onClick={() => setSelected(undefined)}>Close</button></div><dl>{Object.entries(payload || {}).map(([key, value]) => <div key={key}><dt>{key.replace(/([A-Z])/g, ' $1')}</dt><dd>{String(value || '—')}</dd></div>)}</dl></div></div>}{sourceEvidenceId && <SourceEmailEvidenceDrawer stagingId={sourceEvidenceId} onClose={() => setSourceEvidenceId(undefined)} />}<div className="table-wrap"><table><thead><tr><th>Received</th><th>Collection point</th><th>Pallets</th><th>Delivery point</th><th>Source</th><th>Readiness</th><th>Action</th></tr></thead><tbody>{data?.map(item => { const readiness = orderReadiness.get(item.id); return <tr key={item.id}><td>{formatDate(item.receivedAtUtc)}</td><td>{readiness?.collection || '—'}</td><td>{readiness?.pallets || '—'}</td><td>{readiness?.delivery || '—'}</td><td>{item.source || '—'}</td><td>{readiness ? <span className={`order-readiness ${readiness.ready ? 'clear' : 'review'}`} title={readiness.reason}><strong>{readiness.ready ? 'Clear for approval' : 'Needs review'}</strong><small>{readiness.reason}</small></span> : <span className={`status ${statusClass(item.status)}`}>{stagingStatus(item.status)}</span>}</td><td><div className="actions"><button onClick={() => { setSelected(item); setSourceEvidenceId(undefined); }}>Review details</button><button className="approve" disabled={reviewing === item.id} onClick={() => void review(item, true)}>Approve</button><button className="reject" disabled={reviewing === item.id} onClick={() => void review(item, false)}>Reject</button></div></td></tr>; })}</tbody></table></div></State></section>;
}
...
export function Admin() {
  ...
  const fleetioStatus = useApi(useCallback(async () => api.fleetioStatus(await token()), [token]));
  const integrations = useApi(useCallback(async () => api.integrationStatus(await token()), [token]));
  const intake = useApi(useCallback(async () => api.intakeHealth(), []));
  const [syncing, setSyncing] = useState(false);
  const [fleetioSyncing, setFleetioSyncing] = useState(false);
  const [polling, setPolling] = useState(false);
  const [syncMessage, setSyncMessage] = useState<string>();

  async function syncSage() { setSyncing(true); setSyncMessage(undefined); try { const result = await api.syncSageHrDrivers(await token()); setSyncMessage(result.message || `Sage HR sync complete: ${result.created} drivers added, ${result.updated} updated and ${result.skipped} skipped.`); await sage.refresh(); await integrations.refresh(); } catch (exception) { setSyncMessage(exception instanceof Error ? exception.message : 'Sage HR sync failed.'); } finally { setSyncing(false); } }
  async function syncFleetio() { setFleetioSyncing(true); setSyncMessage(undefined); try { const result = await api.syncFleetioVehicles(await token()); setSyncMessage(result.message || `Fleetio sync complete: ${result.updated} vehicles updated, ${result.missingInFleetio} missing in Fleetio.`); await fleetioStatus.refresh(); await integrations.refresh(); } catch (exception) { setSyncMessage(exception instanceof Error ? exception.message : 'Fleetio sync failed.'); } finally { setFleetioSyncing(false); } }
  async function requestOrders() { setPolling(true); setSyncMessage(undefined); try { const result = await api.pollMailboxNow(await token()); setSyncMessage(`${result.message} ${result.lastMessagesIngested} new message(s) staged.`); await intake.refresh(); } catch (exception) { setSyncMessage(exception instanceof Error ? exception.message : 'Graph mailbox poll failed.'); } finally { setPolling(false); } }
  const refreshAdmin = () => { void sage.refresh(); void tacho.refresh(); void fleetioStatus.refresh(); void integrations.refresh(); void intake.refresh(); };
  const health = integrations.data;
  const graph = intake.data?.graph;
  const graphCurrent = Boolean(graph?.configured && !graph.stale && !graph.lastError);
  const samsara = health?.samsara;
  const settings = [
    { label: 'Microsoft sign-in', detail: import.meta.env.VITE_ENTRA_CLIENT_ID ? 'Configured for this portal build' : 'Needs client application ID', status: Boolean(import.meta.env.VITE_ENTRA_CLIENT_ID) },
    { label: 'API connection', detail: import.meta.env.VITE_API_BASE_URL || 'Uses the portal API default', status: true },
    { label: 'RoadTech Falcon', detail: health?.roadTech.connected ? `Live telemetry received ${formatDate(health.roadTech.latestEventUtc)} · automatic tracking` : health?.roadTech.configured ? 'Configured; waiting for a recent telemetry event' : 'RoadTech runtime settings incomplete', status: Boolean(health?.roadTech.connected) },
    { label: 'TachoMaster', detail: tacho.loading ? 'Checking TachoMaster…' : tacho.data?.message || 'TachoMaster status unavailable · scheduled every 20 minutes', status: Boolean(tacho.data?.connected) },
    { label: 'Sage HR', detail: sage.loading ? 'Checking Sage HR…' : sage.data?.message || sage.error || 'Sage HR status unavailable · scheduled daily', status: Boolean(sage.data?.connected) },
    { label: 'Graph mailbox', detail: intake.loading ? 'Checking Microsoft Graph…' : graphCurrent ? `${graph?.mailbox} · last check ${formatDate(graph?.lastSuccessUtc)} · every 60 seconds` : graph?.lastError || 'Graph mailbox polling needs attention', status: graphCurrent },
    { label: 'Fleetio', detail: fleetioStatus.data?.message || (health?.fleetio?.configured ? 'Fleetio service and VOR integration configured · every hour' : 'Fleetio API settings needed'), status: Boolean(fleetioStatus.data?.connected || health?.fleetio?.configured) },
    { label: 'Samsara', detail: samsara?.message || 'Two-way Samsara adapter is not configured in V2 yet.', status: Boolean(samsara?.connected && samsara.twoWay) },
  ];

  return <section><div className="title-row"><div><p className="eyebrow">Platform control</p><h1>Admin & integrations</h1></div><button onClick={refreshAdmin}>Refresh integrations</button></div><p className="intro">Live status comes from the protected API and never exposes credentials.</p><div className="admin-grid">{settings.map(setting => <article className="admin-card" key={setting.label}><span className={setting.status ? 'integration-state ready' : 'integration-state pending'}>{setting.status ? 'Ready' : 'Setup needed'}</span><h2>{setting.label}</h2><p>{setting.detail}</p>{setting.label === 'Graph mailbox' && <><small>{graph?.lastMessagesSeen || 0} seen · {graph?.lastMessagesIngested || 0} newly staged · {intake.data?.pendingReview || 0} pending Order Review</small><button className="primary" disabled={!graphCurrent || polling} onClick={() => void requestOrders()}>{polling ? 'Requesting…' : 'Request orders now'}</button></>}{setting.label === 'Sage HR' && <><small>{sage.data?.employeeCount || 0} employees · {sage.data?.driverCandidateCount || 0} driver candidates</small><button className="primary" disabled={!sage.data?.connected || syncing} onClick={() => void syncSage()}>{syncing ? 'Syncing…' : 'Sync drivers from Sage'}</button></>}{setting.label === 'Fleetio' && <><small>{fleetioStatus.data?.sampleVehicleCount || 0} sample vehicles checked</small><button className="primary" disabled={!fleetioStatus.data?.connected || fleetioSyncing} onClick={() => void syncFleetio()}>{fleetioSyncing ? 'Syncing…' : 'Sync vehicles from Fleetio'}</button></>}</article>)}</div>{syncMessage && <p className="notice">{syncMessage}</p>}<div className="panel admin-next"><h2>Order Review intake</h2><p>Graph checks the Info mailbox every 60 seconds and keeps each captured order in staging before approval.</p><button className="primary" disabled={!graphCurrent || polling} onClick={() => void requestOrders()}>{polling ? 'Requesting orders…' : 'Request orders now'}</button></div></section>;
}
