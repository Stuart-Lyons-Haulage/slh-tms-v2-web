...
export function OrderIntakeMappingAdmin() {
  ...
  async function saveMapping() {
    ...
    try {
      ...
      setMappingId(undefined); setMappingDraft(emptyMapping);
      await load();
  ...
  function editMapping(row: Mapping) {
    ...
    setMode('mappings'); setMessage(undefined);
  }
  ...
  function editRule(row: RouteRule) {
    ...
    setMode('rules'); setMessage(undefined);
  }
  ...
  return <div>
    ...
    {!loading && mode === 'mappings' && <>
      <section className="panel" style={{ marginBottom: 18 }}>
        <div className="title-row"><div><p className="eyebrow">{mappingId ? 'Edit mapping' : 'New mapping'}</p><h2>Sender → customer</h2></div>{mappingId && <button onClick={() => { setMappingId(undefined); setMappingDraft(emptyMapping); }}>Cancel edit</button>}</div>
        ...
        <button className="primary" disabled={saving || !clean(mappingDraft.customerCode) || (!clean(mappingDraft.emailAddress) && !clean(mappingDraft.emailDomain))} onClick={() => void saveMapping()}>{saving ? 'Saving…' : mappingId ? 'Save mapping' : 'Add mapping'}</button>
      </section>
    ...
    {!loading && mode === 'rules' && <>
      <section className="panel" style={{ marginBottom: 18 }}>
        <div className="title-row"><div><p className="eyebrow">{ruleId ? 'Edit route rule' : 'New route rule'}</p><h2>Evidence → route</h2></div>{ruleId && <button onClick={() => { setRuleId(undefined); setRuleDraft(emptyRule); }}>Cancel edit</button>}</div>
        <div className="form-grid">
