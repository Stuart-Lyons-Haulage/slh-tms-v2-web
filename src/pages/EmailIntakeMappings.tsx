...
export function EmailIntakeMappings() {
  ...
  const mappings = useApi(useCallback(async () => request<SenderMapping[]>('/api/v2/customer-email-mappings', await token()), [token]));
  const rules = useApi(useCallback(async () => request<RouteRule[]>('/api/v2/order-intake-route-rules', await token()), [token]));
  ...
  async function saveMapping() {
    ...
    try {
      await request('/api/v2/customer-email-mappings', await token(), { method: 'POST', body: JSON.stringify(clean(mapping)) });
  ...
  async function saveRule() {
    setSaving(true); setMessage(undefined);
    try {
      await request('/api/v2/order-intake-route-rules', await token(), { method: 'POST', body: JSON.stringify(clean(rule)) });
